...
public class SMSKeyManagement extends BroadcastReceiver{
	...
	private byte[] hereIsMyPublicKey = new BigInteger("0xC0DE1FFF").toByteArray();
	private byte[] hereIsMyPublicKeyToo = new BigInteger("0xC0DE2FFF").toByteArray();
	...
	/**
	 * Dovrà essere chiamato alla fine quando un altro oggetto vorrà generare la chiave a partire dal segreto
	 * condiviso.
	 * 
	 * @return
	 */
	...
	private void sendPublicKey(byte[] key, int headerSize, int bodySize){
		...
		this.manager.sendDataMessage(this.other, null, (short)9999, message, null, null);
	}
	
	/**
	 * Metodo chiamato ogni volta che arriva un messaggio: salva messaggio e mittente e chiama il metodo che
	 * parsa il messaggio e agisce di conseguenza.
	 */
	@Override
	 ...
	 */
	private void manageReceivedMessage() throws ArbitraryMessageReceivedException, InvalidKeyException, NoSuchAlgorithmException, InvalidKeySpecException {
		if(receivedMessageStartsWith(this.hereIsMyPublicKey)){
			this.ssa = new SharedSecretAgreement(getPrivateKey(), unwrapContent(this.hereIsMyPublicKey));
			this.ssa.generateSharedSecret();
			this.mPub = getPublicKey();
			sendPublicKey(mPub.getEncoded(), this.hereIsMyPublicKeyToo.length, mPub.getEncoded().length);
		}
		else if(receivedMessageStartsWith(this.hereIsMyPublicKeyToo)){
			this.ssa = new SharedSecretAgreement(getPrivateKey(), unwrapContent(this.hereIsMyPublicKeyToo));
			this.ssa.generateSharedSecret();
		}
