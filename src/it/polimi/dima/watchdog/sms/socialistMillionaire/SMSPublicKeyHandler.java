...
public class SMSPublicKeyHandler extends BroadcastReceiver implements
	...
	public void onReceive(Context context, Intent intent) {
		...
		try {
			if (bundle != null) {
				...
				this.recMsg = this.mSocMilFactory.getMessage(SMSUtility
						.getHeader(message.getUserData()));
	...
	public void visit(PublicKeyRequestCodeMessage pubKeyReqMsg) {
		try {
			...
			this.pka.setMyPublicKey(MyPrefFiles.getMyPreference(
					MyPrefFiles.MY_KEYS, MyPrefFiles.MY_PUB, this.ctx));
	...
	public void visit(PublicKeySentCodeMessage pubKeySentMsg) {
		...
		try {
			...
			// la secret question settata non è in Base64 (è una stringa normale
			// -> vedi AssociateNumberFragment),
			// quindi niente conversione richiesta
			this.pka.setSecretQuestion(MyPrefFiles.getMyPreference(
					MyPrefFiles.SECRET_Q_A, MyPrefFiles.SECRET_QUESTION,
					this.ctx));
			MyPrefFiles.setMyPreference(MyPrefFiles.KEYSQUARE, this.other,
					pubKeySentMsg.getBody(), this.ctx);
	...
	public void visit(SecretAnswerAndPublicKeyHashSentCodeMessage secAnswMsg) {
		try {
			...
			this.pka.setSecretAnswer(MyPrefFiles
					.getMyPreference(MyPrefFiles.SECRET_Q_A,
							MyPrefFiles.SECRET_ANSWER, this.ctx));

			this.pka.setReceivedPublicKey(MyPrefFiles.getMyPreference(
					MyPrefFiles.KEYSQUARE, this.other, this.ctx));
			...
			if (!this.pka.checkForEquality()) {
				...
				MyPrefFiles.setMyPreference(MyPrefFiles.KEYRING, this.other,
						keyValidated, this.ctx);
				String salt = MyPrefFiles.getMyPreference(
						MyPrefFiles.PASSWORD_AND_SALT,
						MyPrefFiles.MY_PASSWORD_SALT, this.ctx);
				SMSUtility.sendMessage(this.other, SMSUtility.SMP_PORT,
						SMSUtility.hexStringToByteArray(SMSUtility.CODE5),
						salt.getBytes());
				String preferenceKey = this.other
						+ MyPrefFiles.ACK_AND_SALT_FORWARDED;
				MyPrefFiles.setMyPreference(MyPrefFiles.SMP_STATUS,
						preferenceKey, this.other, this.ctx);
	...
	private void notifyUser() {
		NotificationCompat.Builder mBuilder = new NotificationCompat.Builder(
				ctx).setSmallIcon(android.R.drawable.stat_notify_chat)
				.setContentTitle("prova").setContentText("hello world")
				.setAutoCancel(true);
